...
def main():
    ...
    while running and not echiquier.is_game_over():
        ...
        for event in pygame.event.get():
            ...
            manager.process_events(event)  # Déplacez cette ligne ici

            # Gestion des événements de l'interface utilisateur
            if event.type == pygame.USEREVENT:
                # Remplacez ce bloc de code dans la boucle des événements:
                if event.type == pygame.USEREVENT:
                    if event.user_type == pygame_gui.UI_DROP_DOWN_MENU_CHANGED:  # Utilisez 'user_type' au lieu de 'type'
                    ...
                    elif event.user_type == pygame_gui.UI_BUTTON_PRESSED:  # Utilisez 'user_type' au lieu de 'type'
                    ...
                    elif event.user_type == pygame_gui.UI_CONFIRMATION_DIALOG_CONFIRMED:  # Utilisez 'user_type' au lieu de 'type'
                        if restart_dialog:
                            echiquier = chess.Board()
                            piece_selectionnee = None
                            restart_dialog = None
                ...
                if event.button == 1:  # Bouton gauche de la souris
                    ...
                    if piece_selectionnee:
                        if piece_selectionnee == (x, y):  # La même pièce a été sélectionnée à nouveau
                            piece_selectionnee = None
                        else:
        ...
        if echiquier.turn == chess.BLACK:
            ...
            mouvement_bot = choisir_mouvement_bot(echiquier)  # Supprimez l'argument 'profondeur' ici
